...
local FFlagUserAddBetterConsoleCheckForLegacyChat
do
...
pcall(function()
	ChatLocalization = require((game:GetService("Chat") :: any).ClientChatModules.ChatLocalization :: any)
end)
...
function HandleChannelJoined(
	...
	if channelObj then
		...
		local translationOnboardingMessage = ChatLocalization:Get(
			"GameChat_ChatMain_ChatTranslationOnboarding",
			"Text chat will be translated into your language. Tap the symbol in front of the message to see the original. You can turn off translations in the Settings menu."
		)

		if
			FFlagUserIsChatTranslationEnabled
			and translationOnboardingMessage ~= ""
			and channel == ChatSettings.GeneralChannelName
			and chatTranslationEnabled ~= nil
			and chatTranslationEnabled.Value
		then

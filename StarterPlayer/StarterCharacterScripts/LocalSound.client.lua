...
do
	...
	local DefaultServerSoundEvent: RemoteEvent = soundEventFolder:FindFirstChild("DefaultServerSoundEvent")
	...
	local stateUpdateHandler = {
		...
		[Enum.HumanoidStateType.Running] = function(speed)
			...
			if activeState == Enum.HumanoidStateType.Freefall and fallSpeed > 0.1 then
				...
				local freeFallSound: Sound = Sounds[SFX.FreeFalling]
		...
		[Enum.HumanoidStateType.Swimming] = function(speed)
			...
			if activeState ~= Enum.HumanoidStateType.Swimming and threshold > 0.1 then
				local splashSound: Sound = Sounds[SFX.Splash]
				splashSound.Volume =
					Util.Clamp(Util.YForLineGivenXAndTwoPts(Util.VerticalSpeed(Head), 100, 0.28, 350, 1), 0, 1)
		...
		[Enum.HumanoidStateType.Freefall] = function()
			...
			local sound: Sound = Sounds[SFX.FreeFalling]
			sound.Volume = 0
			stopPlayingLoopedSoundsExcept()
		...
		[Enum.HumanoidStateType.Landed] = function()
			...
			if Util.VerticalSpeed(Head) > 75 then
				local landingSound: Sound = Sounds[SFX.Landing]
				landingSound.Volume =
					Util.Clamp(Util.YForLineGivenXAndTwoPts(Util.VerticalSpeed(Head), 50, 0, 100, 1), 0, 1)

...
-- local responseTypes = require(game.ReplicatedStorage.types.responsetypes)
...
-- 	pop: responseTypes.popularRaceResult,
-- 	ii: number,
...
-- 			warpRequestFunction:FireServer(pop.startSignId)
-- 			lastCanvasPosition = scrollingFrame.CanvasPosition
-- 			parentSgui:Destroy()
-- 		end)
-- 	end
-- 	if mode == "unfound" then
-- 		local cantwarp = guiUtil.getTl(
-- 			"PopularResult_4_unfound",
...
-- 	local popResults: { responseTypes.popularRaceResult } = getPopularRunsFunction:InvokeServer(userIds)
